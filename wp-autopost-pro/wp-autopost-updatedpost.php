...
if($saction=='queryDuplicate'){
  ...
  if($_POST['query_days']>0){
     $posts = $wpdb->get_results('SELECT id,title FROM '.$t_ap_updated_record.' WHERE date_time > '.(current_time('timestamp')-$_POST['query_days']*24*60*60));
  }else{
     $posts = $wpdb->get_results('SELECT id,title FROM '.$t_ap_updated_record);
  }
...
if($url_status=='notexist'){
  ...
  //$logs = $wpdb->get_results('SELECT t1.*,t3.name FROM '.$t_ap_updated_record.' t1,'.$t_ap_config.' t3 where t1.post_id >0 AND t1.config_id=t3.id AND not exists (select * from '.$wpdb->posts.' t2 where t2.ID = t1.post_id) ORDER BY t1.id DESC LIMIT '.$from.','.$perPage);
...
<?php if($duplicate!='show'){ ?>
 <p class="search-box">
	...
	<input type="search" id="search-input" name="s" size="40" value="<?php echo $_POST['s']; ?>" />
 ...
 <?php $rowNum=0;
  foreach ($logs as $log) { $rowNum++; ?>
   <tr  class="row <?php if($log->url_status==-1)echo 'lightRed'; ?> <?php if($log->url_status==0)echo 'alternate'; ?>"  id="row<?php echo $log->id; ?>"  >
	...
	<?php }else{ ?>
	   <span class="trash"><a href="admin.php?page=wp-autopost-pro/wp-autopost-updatedpost.php&taskId=null"><?php echo __('Has been deleted','wp-autopost'); ?></a></span>
	<?php } ?>
	</td>
	<td>
	...
	<td>
	  <?php if(($log->url_status)==1){ ?>
       <?php
         $postTitle = get_the_title($log->post_id);
         if($postTitle!=null){
		   $postTitle = '<strong><a href="'.get_permalink($log->post_id).'" target="_blank">'.$postTitle.'</a></strong>';
		 }else{
           $postTitle = '[ <span class="trash"><a href="admin.php?page=wp-autopost-pro/wp-autopost-updatedpost.php&url_status=notexist" ><b>'.__('Not Exists','wp-autopost').'</b></a></span> ] '.$log->title;
		 }
	   ?>
	   <?php echo $postTitle; ?>	  
